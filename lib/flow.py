    ...
    def update(self, OVS_duration_flow, OVS_packet_count, OVS_byte_count):
        ...
        if self.next_sample == 1:
            ...
            if OVS_duration_flow < self.request_time:
                ...
                self.pps = (0 if (self.pc == 0 or delta_t == 0) else OVS_packet_count / delta_t)
                self.bps = (0 if (self.bc == 0 or delta_t == 0) else OVS_byte_count / delta_t)
            else:
            ...
            if ((self.OVS_duration_flow > OVS_duration_flow)
            ...
            self.pl_min = (self.pl[-1] if ( ((self.pl[-1] < self.pl_min) and ( self.pl[-1] > 0 )) or (self.pl_min==0)  ) else self.pl_min)

            self.piat_max = max(self.piat)
            # self.piat_min = (self.piat[-1] if ( (self.piat[-1] < piat_min_0) and ( self.piat[-1] > 0 ) ) else piat_min_0)
            self.piat_min = (self.piat[-1] if ( ((self.piat[-1] < self.piat_min) and ( self.piat[-1] > 0 )) or (self.piat_min==0)  ) else self.piat_min)
            ...
            self.pps_min = (self.pps if ( ((self.pps < self.pps_min) and ( self.pps > 0 )) or (self.pps_min==0)  ) else self.pps_min)
            ...
            self.bps_min = (self.bps if ( ((self.bps < self.bps_min ) and ( self.bps > 0 )) or (self.bps_min == 0)  ) else self.bps_min)
